...
export const GET = async ( request: Request ) => { 
    ...
    if (username) {
        try {
            const inqbycode = await db.select().from(inqheadTable).where (and(and(eq(inqheadTable.userName, username), isNull(inqheadTable.deletedAt)), eq(inqheadTable.status,'closed')));

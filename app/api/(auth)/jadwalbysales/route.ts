...
export const GET = async ( request: Request ) => { 
    ...
    if (username) {
        try {
            ...
            const jadwalbycode = await db.select({custCd: jadwalvisitTable.custCd, kodeJadwal: jadwalvisitTable.kodeJadwal, latAkhir: jadwalvisitTable.latAkhir, longAkhir: jadwalvisitTable.longAkhir, custCode: customerTable.custCode, fullName: customerTable.fullName, alamat: customerTable.alamat}).from(jadwalvisitTable).leftJoin(customerTable, eq(jadwalvisitTable.custCd, customerTable.custCode)).where(and(and(eq(jadwalvisitTable.userName, username), isNull(jadwalvisitTable.deletedAt), eq(jadwalvisitTable.tanggal, today))));

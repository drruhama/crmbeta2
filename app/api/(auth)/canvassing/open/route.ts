...
export const GET = async ( request: Request ) => { 
    ...
    if (username) {
        try {
            const canvasbycode = await db.select().from(canvasTable).where (and(and(eq(canvasTable.userName, username), isNull(canvasTable.deletedAt)), isNull(canvasTable.status)));

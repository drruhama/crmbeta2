...
export const GET = async ( request: Request ) => { 
    ...
    if (username) {
        try {
            const quotebycode = await db.select().from(quoteheadTable).where (and(and(eq(quoteheadTable.userName, username), isNull(quoteheadTable.deletedAt)), or(eq(quoteheadTable.approval,'no'),isNull(quoteheadTable.approval))));

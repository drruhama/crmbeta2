...
// Subtract one day from current time                        
    ...
    const yesterday = format(kemarin, 'yyyy-MM-dd')
    console.log('yesterday was:', yesterday)
    
    if (username) {
        try {
            const visitbycode = await db.select({ hitung: count() }).from(kunjunganTable).where(and(and(eq(kunjunganTable.userName, username), isNull(kunjunganTable.deletedAt), eq(kunjunganTable.tglKunjungan, yesterday))));
